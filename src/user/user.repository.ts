...
const nullToUndefined = <T>(object: T) => object === null ? undefined : object
...
export const defaultUserRepository = (userModel: UserModel) => ({
	...
	findByName: (name: string) => ResultAsync.fromPromise(userModel.findOne({name}), mongoExceptionHandler).map(r => r as User).map(nullToUndefined),
	findById: (id: string) => ResultAsync.fromPromise(userModel.findById(id), mongoExceptionHandler).map(r => r as User).map(nullToUndefined),
